...
class Settings:
    ...
    def __init__(self):
        ...
        # Agent configuration
        self.default_allowlist: list[str] = os.getenv(
            "DEFAULT_ALLOWLIST", ""
        ).split(",") if os.getenv("DEFAULT_ALLOWLIST") else []
        self.default_blacklist: list[str] = os.getenv(
            "DEFAULT_BLACKLIST", "rm,dd,format,mkfs,chmod 777"
        ).split(",") if os.getenv("DEFAULT_BLACKLIST") else []

        # Session configuration
        self.app_name: str = os.getenv("APP_NAME", "bash.ai")
        self.user_id: str = os.getenv("USER_ID", "user")
        self.session_id_ask: str = os.getenv("SESSION_ID_ASK", "session-ask")
        self.session_id_agent: str = os.getenv("SESSION_ID_AGENT", "session-agent")
